...
const blogs__content = document.querySelector('.blogs__content')
let container = '';
...
window.addEventListener('load', () => {
    ...
    container += `<div class="blogs__blog">
					...
					<div>
						<form action="" method="">
							<input type="hidden" id="blog__id"value="${index}">
						</form>
						<a href="" class="blog__tn_title">${el.title}</a>
						<p class="blog__tn_description">${el.description.slice(0,150)}... <a href="">Read More</a></p>
					</div>
	...
	if (container.length) {
		blogs__content.innerHTML = container;
	} else {
		blogs__content.innerHTML = 'No Blog ';
	}
})